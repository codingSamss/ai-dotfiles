...
- Claude Code（已支持）
- Codex（暂不支持，依赖 Claude 的 Skill + Agent 执行链路）
...
```bash
./setup.sh cc-codex-review
# 或直接执行
platforms/claude/skills/cc-codex-review/setup.sh
```
...
- 自动检查项：
  - `~/.claude.json` 中是否已注册 `codex` MCP
  - Python3 是否可用
- 需手动补齐项：
  - 未注册 Codex MCP 时，手动执行安装命令
  - 缺少 Python3 时，手动安装 Python3（3.8+）
...
# 关联 Agent（位于 platforms/claude/agents/）
